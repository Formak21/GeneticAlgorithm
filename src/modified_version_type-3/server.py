...
for __ in range(test_quantity):
    ...
    # Modification
    for _ in range(population_quantity + 1):
        individual_bank = []
        machine_bank = []
        ...
        for machine in Connected_Machines:
            if machine in machine_bank:
                data = json.dumps(individual_bank.pop(random.randint(0, len(individual_bank) - 1)))
                machine[0].send(data.encode("utf-8"))
            else:
...
e = math.sqrt(sum([abs(i - test_function.optimal([0] * gene_quantity)) ** 2 for i in solutions[:-1]]) / len(solutions[:-1]))
print(f'E={e}')
